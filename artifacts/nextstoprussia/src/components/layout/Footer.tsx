        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="space-y-4">
            ...
            <p className="text-slate-400 text-sm leading-relaxed max-w-xs">
              Your trusted gateway to studying in Russia. Founded by Dr. Jabroot Khatib to help international students achieve their academic dreams in top Russian universities.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['About', 'Services', 'Universities', 'Fees', 'Admission Process', 'Gallery'].map((item) => (
                <li key={item}>
                  <a href={`#${item.toLowerCase().split(' ')[0]}`} className="text-slate-400 hover:text-white transition-colors text-sm flex items-center">
          ...
          {/* Services */}
          <div>
            <h3 className="text-white font-bold text-lg mb-4 font-display">Top Programs</h3>
            <ul className="space-y-3">
              {['MBBS in Russia', 'Engineering', 'Aviation Courses', 'Business & Management', 'Language Preparatory'].map((item) => (
                <li key={item}>
                  <a href="#services" className="text-slate-400 hover:text-white transition-colors text-sm flex items-center">
                    <span className="mr-2 text-primary">›</span>
                    {item}
                  </a>
                </li>
